...
function EditRecipe() {
  ...
  function fetchRecipe() {
    return fetch(process.env.REACT_APP_BACKEND_URL + "/api/recipes/" + recipeId)
      ...
      .then((data) => {
        setRecipe(data);
        setIngredients(data.ingredients.map((ingredient) => [uuidv4(), ingredient]));
        ...
        >
          ...
          <form
            onSubmit={async (e) => {
              ...
              data.append('categoriesId', e.target.elements.categoriesId.value);
              ...
              data.append('recipeImage', e.target.elements['recipeImage'].files[0]);

              await fetch(process.env.REACT_APP_BACKEND_URL + "/api/recipes/" + recipeId, {
                method: 'PUT',
                body: data
              });
              await fetchRecipe();
          ...
          >
            ...
            <div className="mb-3">
              ...
              {ingredients.map(([ingredientId, ingredient], index) => (
                <div key={ingredientId} className="offset-1">
                  <div
                    className={
                      "row mb-3 " +
                      (index === ingredients.length - 1 ? "" : "border-botton")
                    }
                  >
